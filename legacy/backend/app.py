...
class Draft:
    ...
    def makePick(self, newPlayer:Player, pick:Pick) -> bool:
        if not database.checkForPlayer(pick):
            return False
        ...
        if draft.key == draftKey:
            ...
            for player in draft.players:
                if player.key == userKey:
                    ...
                    if draft.makePick(player, Pick(
                        playerName,
                        Position(pos),
                        Team(team)
                        )
                    ): return jsonify({"status": "Success - player added"}), 200 

                    else:
                        return jsonify({"status": "fail - could not make pick"}), 404 #To-Do: say why no pick (ie. not user's turn)
...
def showDraftState():
    ...
    for draft in drafts:
        ...
        for p in draft.players:
            if draft.players.index(p) == draft.playerOnTheClock:
